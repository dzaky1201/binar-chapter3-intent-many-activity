...
class Screen3Activity : AppCompatActivity() {
    ...
    fun viewIdentity(identity: Identity) {
        ...
        if (identity.usia % 2 == 0) {
            binding.viewDetail4.txtAge.text = getString(R.string.umur, identity.usia, "Genap")
        } else {
            binding.viewDetail4.txtAge.text = getString(R.string.umur, identity.usia, "Ganjil")
        }
    }
}